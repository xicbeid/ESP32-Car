/*
 * IMU Parser — Wit-Motion Normal Protocol (C implementation)
 *
 * Ported from Wit-Motion imu_usb.py (V1.5.1) and wit_c_sdk.c
 * Additions: EMA low-pass filter for vibration rejection on moving platform.
 */
...
/* EMA filter alpha (0 < alpha <= 1, smaller = smoother) */
...
static struct {
    ...
    uint8_t  idx;          /* Write position in buf */
    bool     synced;       /* Found 0x55 header */
    imu_data_t raw;        /* Latest raw parsed data */
    imu_data_t filtered;   /* EMA-filtered output */
...
static void process_frame(void)
{
    ...
    /* Verify checksum: sum of bytes 0..9, low 8 bits */
    uint8_t sum = 0;
    ...
    /* Extract 4 int16 values (low byte first) */
    ...
    if (upd) {
        ...
        /* Apply EMA filter to the most vibration-prone channels */
        ...
        s_ctx.filtered.acc_z  = s_ctx.raw.acc_z;  /* gravity axis — keep raw */
        ...
        s_ctx.filtered.yaw    = s_ctx.raw.yaw;  /* heading — keep raw */
